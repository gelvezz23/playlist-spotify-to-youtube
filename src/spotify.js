...
async function tokenRequest(body) {
  ...
  if (!res.ok) throw new Error(data.error_description || data.error || 'Error de Spotify');
  saveTokens(data);
...
export async function handleSpotifyRedirect() {
  ...
  if (!verifier) throw new Error('Falta el code_verifier de Spotify, intenta de nuevo');
  return tokenRequest({
...
export async function getSpotifyToken() {
  ...
  try {
    return await tokenRequest({ grant_type: 'refresh_token', refresh_token: refresh });
  } catch {
    logoutSpotify();
    return null;
  }
